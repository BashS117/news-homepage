    ...
    const Top = [
        {
            ...
            description: 'What happends when old PCs aregiven modern upgrades'
    ...
    <aside className='tier bg-red py-6 flex gap-[30px] sm:flex-col'>
        ...
        {Top.map((content,index)=>(
            <div key={content.title} className='flex w-[450px] sm:w-auto'>
                <figure>
                    <img width='100px' src={content.image} alt="" />
                </figure>
                <div className='w-[240px] ml-6'>
                <span className='text-[2rem] font-bold text-[var(--Grayish-blue)]'>0{index+1}</span>
                <h2 className='hover:text-[var(--Soft-red)] mb-2 text-[var(--Very-dark-blue)] font-bold text-[1.2rem]'>{content.title}</h2>
                <p className='font-normal'>{content.description}</p>
                </div>
